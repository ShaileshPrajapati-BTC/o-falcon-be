...
class ESBankDataUpsertModal extends React.Component {
    ...
    validateFile = function (file, as) {
        ...
        if (validSize) {
            message.error('file must be less then 5MB');
    ...
    handleSubmit = async () => {
        ...
        form.validateFields(async (err, values) => {
            ...
            if (obj.path === '' || obj.path === undefined) {
                message.error('Document Image must be upload')
                return
            }
    ...
    render() {
        ...
        return (
            <Modal
                visible={true}
                title={<IntlMessages id="app.partner.addDocument" defaultMessage="Add Document" />}
                okText={<IntlMessages id="app.add" defaultMessage="Add" />}
                ...
                <Form layout="vertical">
                    <Row type="flex" justify="start">
                        <Col lg={12} md={12} sm={12} xs={24}>
                            <Form.Item label={<IntlMessages id="app.partner.documentType" defaultMessage="Document Type" />} hasFeedback>
                                {getFieldDecorator("type", {
                                    rules: [{
                                        required: true,
                                        message: <IntlMessages id="app.partner.documentTypeRequiredMsg" defaultMessage="Please select Document Type!" />
                                    }]
                                })(
                                    <Select
                        ...
                        {this.state.visibleDocumentName &&
                            <Col lg={12} md={12} sm={12} xs={24}>
                                <Form.Item label={<IntlMessages id="app.partner.documentName" defaultMessage="Document Name" />}
                                    style={{ paddingLeft: '5px' }}
                                    hasFeedback>
                                    {getFieldDecorator('name', {
                                        rules: [{
                                            required: true,
                                            message: <IntlMessages id="app.partner.documentNameRequiredMsg" defaultMessage="Please add Document Name!" />
                                        }, { max: 25 },
                                        {
                                            pattern: /^([a-zA-Z0-9]+)$/,
                                            message: <IntlMessages id="app.partner.documentNameSpaceMsg" defaultMessage="No space allowed!" />
                                        }]
                                    })(
                                ...
                                {getFieldDecorator('path', {
                                    ...
                                    <Upload key="image" {...this.state.imageUploadprops}>
                                        <Button disabled={this.state.disable}>
                                            <Icon type="upload" /> <IntlMessages id="app.partner.uploadDocument" defaultMessage="Upload Document" />
                                        </Button>
                                    </Upload>
                                )}
                            </Form.Item>
