...
const ackStructure = {
    ...
    eventIOId: { start: 68, end: 2 },
    nOfTotalId: { start: 70, end: 2 },
    n1OfOneByte: { start: 72, end: 2 },
    ioId: { end: 2 },
    noOfData2: { end: 2 },
    crc16: { end: 8 }
};
...
module.exports = {
    decodeAcknowledgement(ack) {
        ...
        if (ack.substr(ackStructure.noOfData1.start, ackStructure.noOfData1.end) == ack.substr(ack.length - 10, ackStructure.noOfData2.end)) {
            ...
            ackData.lng = UtilService.hexToDec(hexValue) / 10000000;

            hexValue = ack.substr(ackStructure.latitude.start, ackStructure.longitude.end);
            ackData.lat = UtilService.hexToDec(hexValue) / 10000000;
            ...
            throw Error('Acknowledgement Data is not verified.');
        }
