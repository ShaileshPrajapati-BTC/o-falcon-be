...
function _generateParameters(config, context) {
  ...
  params.LimitQueryParam = {
    ...
    type: "integer",
    description:
      "The maximum number of records to send back (useful for pagination). Defaults to " +
      config.blueprints.defaultLimit
  };
...
function _generateRoutes(controllers, config, tags) {
  ...
  _.forEach(controllers, function(controller, identity) {
    if (!routes[identity]) routes[identity] = [];
    ...
    var prefix =
      (blueprints.prefix && blueprints.prefix !== "" ? blueprints.prefix : "") +
      (blueprints.restPrefix && blueprints.restPrefix !== ""
        ? blueprints.restPrefix
        : "");
    if (prefix !== "") prefix += "/";
    if (blueprints.rest === true) {
      //if rest is true add this to default routes and actions
      ...
      if (_.find(tags, { identity: identity })) {
        controller = _.merge(controller, {
          findOne: {
            ...
            description:
              "This is use to get a single {identity} with the supplied id"
          },
          find: {
            http_method: "get",
    ...
    _.forEach(controller, function(value, action) {
      if (exclude.indexOf(action) > -1) return;
...
function _generatePaths(generated_routes, tags, customBlueprintMaps) {
  ...
  _.forEach(generated_routes, function(routes, identity) {
    _.forEach(routes, function(route) {
      ...
      if (!route.custom) {
        // if not from routes.js use the default custom blueprint maps
        parameter = formatter.blueprint_parameter(
          route.action,
          customBlueprintMaps
        );
      }
      ...
      if (!parameter) {
        //which means is not in our parameters, then use the keys to get and add token for us incase of any route using header information
        parameter = _getPathParamsFromKeys(route.keys).concat({
      ...
      var path = {
        summary: route.summary
          ? route.summary.replace("{identity}", _.capitalize(identity))
          : "",
        description: route.description
          ? route.description.replace("{identity}", _.capitalize(identity))
          : "",
        tags: tag ? [tag.name] : [identity],
        produces: ["application/json"],
        consumes: ["application/json"],
        parameters: parameter
          .concat(_getQueryParamsFromParameters(route.parameters))
        ...
        responses: {
          ...
          "400": {
            description: "The requested resource has bad request params"
          },
      ...
      if (route.action === "create" || route.action === "update") {
        ...
        if (!_.find(path.parameters, { name: "body" })) {
          path.parameters.push({
            ...
            required: true,
            description:
              "An object defining our schema for " + _.capitalize(identity),
      ...
      var link = _standardiseLink(
        route.path + "/" + _processRouteKeys(route.keys)
      );
      if (!paths[link])
        //if it's undefined create it as empty object for us
        paths[link] = {};
...
function _standardiseLink(link) {
  ...
  if (!_.startsWith(link, "/")) link = "/" + link;

  if (_.endsWith(link, "/")) link = link.slice(0, -1); //remove the / by the end
...
function _getQueryParamsFromParameters(parameters) {
  ...
  _.forEach(parameters, function(parameter) {
    params.push({
      ...
      description:
        parameter.description || "This is a custom param for " + parameter.name
    });
  });
