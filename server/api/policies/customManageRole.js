...
module.exports = (req, res, next) => {
    if (req.headers && req.headers.authorization) {
        passport.authenticate('jwt', async (error, user, info) => {
            ...
            if (user.type === sails.config.USER.TYPE.FRANCHISEE || user.type === sails.config.USER.TYPE.DEALER) {
                if (params.filter) {
                    req.body.filter.referenceId = user.id;
                    delete req.body.filter.userId;
                }
                req.body.updateFilter = { referenceId: user.id };
                req.body.deleteFilter = { referenceId: user.id };
                req.body.viewFilter = { referenceId: user.id };
            } else {
